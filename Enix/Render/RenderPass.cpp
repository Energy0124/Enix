...
namespace Enix {
    ...
    void RenderPass::createImGuiRenderPass() {
        VkAttachmentDescription colorAttachment{};
        colorAttachment.format = _swapChainImageFormat; // Same as your swapchain format
        colorAttachment.samples = VK_SAMPLE_COUNT_1_BIT;
        colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD; // Load to avoid clearing main render
        colorAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
        colorAttachment.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
        colorAttachment.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
    ...
    RenderPass::RenderPass(Device &device) : RenderPass(device, false) {
    }

    RenderPass::RenderPass(Device &device, const bool isImgui) : _device(device) {
        if (isImgui) {
